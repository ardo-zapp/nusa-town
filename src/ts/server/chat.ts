...
function sayTransientMessage(
	...
	if (isPartyChat(type)) {
		sayToParty(client, message, think ? MessageType.PartyThinking : MessageType.Party, false);
	} else if (whisper) {
		sayWhisper(client, message, message, getMessageType(client, type), target, settings, false);
	} else {
...
function sayToClient(
	...
	if (client.pony !== entity && !isWhisperTo(type) && !isTypingIndicatorMessage(message)) {
