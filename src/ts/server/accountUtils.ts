...
async function hasDuplicatesAtOrigin(account: IAccount, ip: string) {
	...
	const duplicates = await Account.find(query, '_id ban mute shadow flags name').lean().exec();
...
async function findFriendRequest(accountId: string, friendId: string): Promise<IFriendRequest | undefined> {
	const requests = await FriendRequest.find({
		$or: [
			{ source: accountId as any, target: friendId as any },
			{ source: friendId as any, target: accountId as any },
		]
	}).exec();
