...
if (args.login || args.admin) {
	passport.serializeUser<any, any>((account: any, done: any) => done(null, account._id.toString()));
	passport.deserializeUser<any, any>((id: any, done: any) =>
...
const host = createServerHost(httpServer, {
	...
	ws: {
		Server: function(options: any) {
			uwsAdapter = new UwsAdapterServer(options);
			return uwsAdapter;
		}
	},
...
if (args.admin) {
	...
	const adminSocket = host.socket(AdminServerActions, ClientAdminActions, createClient, {
		...socketOptionsBase,
...
if (args.login) {
	...
	const csp = `object-src 'none';`
		...
		+ `script-src 'self' 'unsafe-inline' 'unsafe-eval' ${script} ${scriptES} ${analytics};`
		// + `report-uri /api2/csp`
		;
	...
	httpServer.listen(port, () => {
		if (uwsAdapter) {
			uwsAdapter.listen(wsPort, () => {
				...
				logger.info(`Listening on port ${port} (HTTP) and port ${wsPort} (WS) (${options.join(', ')})`);
			});
		} else {
			logger.error(`uwsAdapter was not initialized. WebSocket server will not work.`);
		}
	});
});
