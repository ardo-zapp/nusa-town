...
import { ISupporterInvite } from '../db';
...
export async function updateSupporterInvites(model: Model<ISupporterInvite>) {
	...
	await Promise.all(groups.map(([_, items]) => {
		...
		return model.updateMany({ _id: { $in: ids as any } }, { active } as any).exec();
	}));

	await model.deleteMany({ active: false, updatedAt: { $lt: fromNow(-100 * DAY) } } as any).exec();
}
