...
const createLogEvent =
	(config: ServerConfig) =>
		(
			...
			Event.findOne({ server, account, pony, type, message, origin } as any).exec()
				.then((event: any) => {
					if (event) {
						...
						return Event.updateOne({ _id: event._id }, { desc: event.desc, count: event.count + 1 }).exec() as any;
					} else {
						return Event.create(<IEvent>{ server, account, pony, type, message, origin, desc }) as any;
					}
				})
				.catch(logger.error);
