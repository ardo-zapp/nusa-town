...
export function createLiveEndPoint<T extends Doc>(
	...
	function assignAccount(id: string, account: string) {
		return Promise.resolve()
			...
			.tap(() => model.findByIdAndUpdate(id, { account } as any).exec())
	...
	function findItems(from: Date): Promise<T[]> {
		return Promise.resolve(model.find({ updatedAt: { $gt: from } } as any, fields.join(' '))
	...
	function findItemsExact(date: Date): Promise<T[]> {
		return Promise.resolve(model.find({ updatedAt: date } as any, fields.join(' '))
			.lean()
			.exec() as any);
	}
	...
	function fixItems(items: T[]) {
		...
		Promise.map(items, item => model.updateOne({ _id: item._id as any }, { unused: Date.now() % 1000 } as any).exec(), { concurrency: 1 })
