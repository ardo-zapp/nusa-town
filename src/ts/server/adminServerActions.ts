...
import {
	IAccount, Account, Character, Auth, checkIfAdmin, ID, findCharacterById, updateAuth, queryAuths,
...
export class AdminServerActions implements IAdminServerActions, SocketServer {
	...
	async getPoniesCreators(account: string) {
		const items = await Character.find({ account }, '_id name creator').lean().exec();
		return items.map(({ _id, name, creator }) => <PonyCreator>{ _id, name, creator });
	}
	@Method({ promise: true })
	async getPoniesForAccount(account: string) {
			return await Character.find({ account } as any).lean().exec() as any;
	}
	@Method({ promise: true })
	...
	async getIgnoresAndIgnoredBy(accountId: string): Promise<{ ignores: string[]; ignoredBy: string[]; }> {
		const [ignores, ignoredBy] = await Promise.all([
			Account
					.find({ ignores: { $in: [accountId] as any } } as any, '_id')
				.lean()
				.exec()
					.then((accounts: any[]) => accounts.map(a => a._id.toString())),
			Account
					.findOne({ _id: accountId } as any, 'ignores')
				.lean()
				.exec()
					.then((account: any) => account && account.ignores || []),
		]);
