...
export async function getDuplicateInfo(accountId: string, otherAccounts: string[]): Promise<DuplicateInfoEntry[]> {
	...
	const [chars, accounts] = await Promise.all([
			Character.find({ account: { $in: ids as any } }, 'account name').lean().exec() as Promise<ICharacter[]>,
			DBAccount.find({ _id: { $in: ids as any } }, '_id lastUserAgent').lean().exec() as Promise<IAccount[]>,
	]);
