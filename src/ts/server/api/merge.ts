...
async function merge(
	...
	const update: Partial<AccountBase<string>> = {
		origins, ignores, emails, note, lastVisit, ban, shadow, mute, flags, counters, patreon, supporter, merges: merges as any,
		createdAt, supporterLog: supporterLog as any, supporterTotal, banLog: banLog as any, state, alert, birthdate,
	};
...
async function removeDuplicateFriendRequests(id: string) {
	const friendRequests = await FriendRequest.find({ $or: [{ source: id as any }, { target: id as any }] }).exec();
...
export async function split(
	...
	await Promise.all([
		...
		Account.updateOne({ _id: account._id } as any, accountUpdate as any).exec(),
	]);

	// friends

	const friendsToRemove = [...(keep.friends || []), ...(split.friends || [])] as any[];

	await FriendRequest.deleteMany({
		$or: [
			{ target: account._id, source: { $in: friendsToRemove } as any },
			{ source: account._id, target: { $in: friendsToRemove } as any },
		] as any[],
	}).exec();
	...
	const hidesToRemove = [...(keep.hides || []), ...(split.hides || [])].map(hide => hide.id) as any[];

	await HideRequest.deleteMany({
		$or: [
			{ source: account._id, target: { $in: hidesToRemove } as any },
		] as any[],
	}).exec();
