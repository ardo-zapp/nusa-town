...
export function iterate<T>(query: any, onData: (doc: T) => void) {
...
export const findAllCharacters: FindCharacters = (account, fields) =>
	Character.find({ account } as any, fields).lean().exec() as any;

export function findLatestCharacters(account: ID, count: number): Promise<ICharacter[]> {
	return Character.find({ account } as any)
		.sort('-lastUsed')
		.limit(count)
		.exec() as any;
}
...
export const updateCharacterState: UpdateCharacterState = (characterId, serverName, state) =>
	Character.updateOne({ _id: characterId }, { [`state.${serverName}`]: state } as any).exec().then(() => {});

export const queryCharacter: QueryCharacter = (query, fields) =>
	Character.findOne(query as any, fields).exec() as any;
...
export const findAllVisibleAuths: FindAuths = (account, fields) =>
	Auth.find({ account, disabled: { $ne: true }, banned: { $ne: true } } as any, fields).lean().exec() as any;

export const countAllVisibleAuths: CountAuths = (account) =>
	Auth.find({ account, disabled: { $ne: true }, banned: { $ne: true } } as any).countDocuments().exec();

export const queryAuths: QueryAuths = (query, fields) =>
	Auth.find(query as any, fields).lean().exec() as any;

export const updateAuth: UpdateAuth = (id, update) =>
	Auth.updateOne({ _id: id } as any, update as any).exec() as any;
...
export const updateAccount: UpdateAccount = (accountId, update) =>
	Account.updateOne({ _id: accountId } as any, update as any).exec() as any;

export const updateAccounts: UpdateAccounts = (query, update) =>
	Account.updateMany(query as any, update as any).exec() as any;

export const queryAccounts: QueryAccounts = (query, fields) =>
	Account.find(query as any, fields).lean().exec() as any;

export const queryAccount: QueryAccount = (query, fields) =>
	Account.findOne(query as any, fields).exec().then(nullToUndefined);
...
export async function findFriendIds(accountId: ID) {
	...
	const friendRequests = await FriendRequest
		.find({ $or: [{ source: accountId as any }, { target: accountId as any }] }, 'source target')
		.lean()
		.exec();
...
export async function findFriends(accountId: ID, withCharacters: boolean): Promise<FriendData[]> {
	...
	const accounts = await Account.find({ _id: { $in: friendIds as any } }, '_id name lastOnline lastCharacter').lean().exec() as any[];
	let characters: ICharacter[] = [];

	if (withCharacters) {
		const characterIds = accounts.map(a => a.lastCharacter).filter(id => id);
		characters = await Character.find({ _id: { $in: characterIds as any } }, '_id name info').lean().exec() as any[];
	}
...
export async function findHideIds(accountId: ID) {
	const hideRequests = await HideRequest.find({ source: accountId } as any, 'target').lean().exec() as any[];
	return hideRequests.map(f => f.target.toString());
}

export async function findHideIdsRev(accountId: ID) {
	const hideRequests = await HideRequest.find({ target: accountId } as any, 'source').lean().exec() as any[];
...
export async function findHidesForMerge(accountId: ID): Promise<MergeHideData[]> {
	const hideRequests = await HideRequest
		.find({ source: accountId } as any, '_id name date')
		.lean()
		.exec() as any[];
