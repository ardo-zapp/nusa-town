...
function test(file: string, draw: (batch: SpriteBatch & PaletteSpriteBatch) => void) {
	return () => {
		...
		try {
			...
			if (e.message === 'Image given has not completed loading' || e.message === 'Unsupported image type') {
				console.warn('Skipping graphics test (likely git-lfs placeholder): ' + file);
			} else {
				throw e;
			}
		}
