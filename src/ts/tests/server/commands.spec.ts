	...
	describe('individual commands', () => {
		...
		describe('collectable announcements', () => {
			it('announces gift milestone at 100', () => {
				...
				expect(client.saysQueue.some(s => s[1].includes('Congratulations') && s[2] === MessageType.Announcement)).true;
			});
		...
		describe('/collect', () => {
			it('modifies counters and notifies admin', () => {
				...
				expect(client.saysQueue.some(s => s[1].includes('You granted') && s[2] === MessageType.System)).true;
			});
