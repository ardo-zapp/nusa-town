...
describe('SupporterInvitesService', () => {
	...
	describe('invite()', () => {
		...
		assert.calledWithMatch(model.create as any, sinon.match({
	...
	describe('uninvite()', () => {
		it('removes invite', async () => {
			...
			await service.uninvite(requester, 'foobar');
				...
				assert.calledWithMatch(model.deleteOne as any, sinon.match({ _id: 'foobar', source: requester.account._id }));
		});
	});
	...
	describe('updateSupporterInvites()', () => {
		...
		it('removes old inactive entries', async () => {
			...
			assert.calledWithMatch(model.deleteMany as any, sinon.match({ active: false }));
		});
	});
});
