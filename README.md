...
> Technically, this repository can be used to build a Pony Town custom server, but our roadmap is committed to systematically refactoring the original logic in favor of Nusantara's local wisdom and tropical reality.
>
> Buat developer Indonesia, silakan pelajari kodenya. Buat lu orang bule, mau ngapain di sini? Ikutan nyawit juga?

<p align="center"><img src="public/images/nyawit_nih_orang.png" width="300" style="max-width: 100%;" alt="Orang Nyawit"></p>
...
This is a custom server project for Pony Town, featuring a localized Indonesian (Nusantara) theme. The project is designed as a foundation that you can further modify and customize to suit your community's needs.

**NOTE:** This repository is based on the legacy Pony Town v0.55.2 source code (which has since become closed source). Originally running on Node.js version 9, the entire codebase has been modernized to support **Node.js 24 LTS**.
...
3. [Before you clone](#3-before-you-clone)
4. [Installation](#4-installation)
5. [Setup Configuration](#5-setup-configuration)
6. [Running the Server](#6-running-the-server)
7. [Service Worker / Progressive Web App (PWA)](#7-service-worker--progressive-web-app-pwa)
8. [Deployment & Production](#8-deployment--production)
9. [Customization & Advanced Modifications](#9-customization--advanced-modifications)
10. [Repo Quirks](#10-repo-quirks)
11. [In-Game Chat Commands](#11-in-game-chat-commands)
...
* System dependencies for Canvas:
  ```bash
  sudo apt update
  sudo apt install build-essential libcairo2-dev libpango1.0-dev libjpeg-dev libgif-dev librsvg2-dev pkg-config
  ```
...
**Ensure you have Git LFS installed before you clone the repo!** Git LFS (Large File Storage) is required to pull down the asset files the game uses, as storing them in normal Git would stuff up the tree pretty badly.
...
```json
{
  ...
  "twitterLink": "<LINK_TO_TWITTER>", // URL to your X (Twitter) account
  ...
  "wdsUrl": "http://localhost:8091", // Webpack Dev Server URL used during local development (yarn wds)
...
Get OAuth keys for the authentication platform of your choice (github, google, twitter, vkontakte, discord, steam).
...
**Steam**
- Go to https://steamcommunity.com/dev/apikey to generate a Steam Web API Key.
- Add this to the `oauth` field in your `config.json`:
  ```json
  "steam": {
...
```bash
yarn start                       # Starts login, admin, and the "main" game server
...
```bash
node pony-town.js --login --beta             # Login server (--beta is optional. Only works if you have run "yarn build-beta", and the server has "test" & "editor" flag)
...
2. Build Dependency: The `--beta` argument only works if you have run `yarn build-beta`. If you run `yarn build` or `yarn build-fast`, the beta features will be automatically removed. You have to run `yarn build-beta` again to enable it.

#### Development Environment

```bash
yarn ts-watch        # terminal 1
yarn wds             # terminal 2
...
| Command | Description |
|---|---|
| `yarn start` | Starts all servers (login, admin, and game main) simultaneously using the default config. |
| `yarn startlocal` | Starts all servers passing the `--local` flag to enable loopback development mode. |
| `yarn build` | Full production build of the frontend, maps, and tools. |
| `yarn build-fast` | Runs a production build faster using parallel workers. |
| `yarn build-beta` | Builds the frontend including debugging tools (for beta environments). |
| `yarn wds` | Starts the Webpack Dev Server to compile and serve frontend changes in real-time (usually on port 8091). |
| `yarn ts-watch` | Watches and live-compiles backend TypeScript (`src/ts`) files into JavaScript (`src/scripts`). |
| `npx gulp dev --sprites` | Compiles image assets into usable sprite sheets. |
| `yarn test-ts` | Executes all mocha `.spec.ts` unit tests directly using ts-node. |
| `yarn lint` | Runs the TSLint checking to ensure code style compliance. |
| `yarn sw` | Generates and minifies the PWA service worker files (`build/sw.js`). |
...
1. You must set proxy to `true`:
   ...
   ```
2. Dev Server, this part is optional, you can leave it for map editing, if left as is don't forget to run "yarn build-beta".
...
And run the standard build (not beta) "yarn build" or "yarn build-fast".

Make sure you have run "yarn build-beta" if you keep the dev server or "yarn build-fast" if you have deleted the dev server from config.json.
...
2. **Run the server cluster**:
   Make sure you have built the server (`yarn build-fast`) first, then run:
   ```bash
...
### 9. Customization & Advanced Modifications
...
By default, Docker uses "yarn build-fast". Please adjust the Dockerfile in the /docker/ directory to "yarn build-beta" if you want to run beta features (--beta) and tools (/tools/).
...
**CATATAN:** Proyek ini mengambil basis dari Pony Town versi lawas (v0.55.2). Hal ini dikarenakan pengembang utama Pony Town sudah berhenti merilis pembaruan kode publik dan secara resmi telah menjadi *closed source*. Pada kode aslinya, proyek ini hanya bisa berjalan menggunakan Node.js versi 9, namun repositori Nusa Town ini telah diperbarui sehingga Anda dapat menjalankannya di lingkungan modern menggunakan **Node.js 24 LTS**.
...
3. [Sebelum Anda Clone](#3-sebelum-anda-clone)
4. [Instalasi](#4-instalasi)
5. [Pengaturan Konfigurasi](#5-pengaturan-konfigurasi)
...
9. [Modifikasi Lanjutan](#9-modifikasi-lanjutan)
10. [Repo Quirks](#10-repo-quirks-1)
11. [Perintah Obrolan (In-Game Chat Commands)](#11-perintah-obrolan-in-game-chat-commands)
12. [Menjalankan dengan Docker](#12-menjalankan-dengan-docker)

---

### 1. Lisensi

Kode sumber Pony Town dirilis ke domain publik (*public domain*). Aset gambar/seni dan musik dirilis di bawah lisensi non-komersial CC BY-NC 4.0. Semua aset ini sama sekali tidak boleh digunakan untuk tujuan komersial dalam bentuk apa pun, termasuk penggalangan dana seperti Patreon atau sumbangan PayPal tanpa ada izin. Silakan baca berkas `LICENSE_CODE.txt`, `LICENSE_ART.txt`, dan `LICENSE_MUSIC.txt` untuk penjelasan lebih lanjut. Untuk mendiskusikan masalah lisensi, hubungi `ponytownhelp@gmail.com`.
...
### 3. Sebelum Anda Clone

**Pastikan Anda sudah menginstal Git LFS sebelum melakukan kloning (clone) pada repositori ini!** Git LFS (Large File Storage) wajib digunakan agar file-file aset gambar berukuran besar yang dipakai dalam gim bisa diunduh dengan benar. Jika aset sebesar ini ditaruh di dalam Git biasa, ukuran repositori akan menjadi sangat bengkak dan lambat.

Anda dapat menginstalnya di Ubuntu/Debian menggunakan perintah:
```bash
sudo apt update
sudo apt install git-lfs
git lfs install
...
```json
{
  ...
  "twitterLink": "<LINK_TO_TWITTER>", // Tautan ke akun X (Twitter) Anda
  ...
  "wdsUrl": "http://localhost:8091", // URL Webpack Dev Server untuk keperluan development lokal (yarn wds)
...
Jika masalah sudah teratasi dan generator sprite sudah benar-benar berfungsi dengan memakai aset gambar yang asli, hapus saja file sementara tadi (atau biarkan ditimpa langsung oleh perintah generator), lalu jalankan ini untuk menjaga repositori Git Anda tetap bersih:

```bash
git update-index --assume-unchanged src/ts/generated/sprites.ts
```

Perintah di atas berguna agar modifikasi otomatis pada file lokal `sprites.ts` tidak selalu tercatat sebagai "perubahan" merah oleh Git.
...
Dapatkan kunci OAuth untuk platform pilihan Anda (github, google, twitter, vkontakte, discord, steam).
...
**Steam**
- Buka https://steamcommunity.com/dev/apikey untuk mendapatkan Steam Web API Key Anda.
- Tambahkan ini ke field `oauth` di `config.json` Anda:
  ```json
  "steam": {
    "apiKey": "<api_key_steam_anda>"
  }
  ```
...
Server ini telah memodifikasi struktur arsip dari Pony Town asli sehingga aset gim yang sudah dikompilasi tidak lagi disertakan di dalamnya. Untuk memproses (mengompilasi) aset gim ini pertama kalinya, jalankan perintah di bawah. Perintah ini akan menghasilkan *spritesheets* dan data terkait yang dibutuhkan, **sekaligus** melakukan proses build produksi secara penuh untuk situs web Anda:
...
**Namun, jika Anda hanya ingin memproses (build) gambarnya saja** (biasanya digunakan saat masih tahap pengembangan sebelum server dijalankan), Anda bisa memakai perintah ini:
...
```bash
yarn start                       # Memulai server login, admin, dan "main" game secara bersamaan
...
```bash
node pony-town.js --login --beta             # Menjalankan server Login (--beta bersifat opsional. Hanya berfungsi jika Anda sudah menjalankan "yarn build-beta", dan server memiliki flag "test" & "editor")
...
2. Ketergantungan Build: Argumen `--beta` hanya berfungsi jika Anda sudah menjalankan `yarn build-beta`. Jika Anda menjalankan `yarn build` atau `yarn build-fast`, fitur beta akan terhapus otomatis. Anda harus menjalankan `yarn build-beta` kembali untuk mengaktifkannya.
...
| Perintah | Fungsi |
|---|---|
| `yarn start` | Memulai semua server sekaligus (login, admin, dan game main) menggunakan konfigurasi default. |
| `yarn startlocal` | Memulai semua server dengan flag `--local` (mode development lokal/loopback). |
| `yarn build` | Melakukan proses build penuh untuk frontend, peta, dan alat secara production. |
| `yarn build-fast` | Menjalankan build secara paralel agar jauh lebih cepat. |
| `yarn build-beta` | Melakukan build aplikasi dengan environment beta dan menyertakan tools inspector. |
| `yarn wds` | Memulai Webpack Dev Server untuk kompilasi dan reload frontend secara langsung (biasanya pada port 8091). |
| `yarn ts-watch` | Menjalankan compiler Typescript (mengawasi `src/ts`) ke JavaScript (`src/scripts`) secara real-time. |
| `npx gulp dev --sprites` | Mengompilasi dan mengompres aset gambar menjadi sprite sheets yang dipakai game. |
| `yarn test-ts` | Menjalankan seluruh berkas unit test Mocha (`.spec.ts`) secara langsung memakai ts-node. |
| `yarn lint` | Memeriksa ketaatan standar gaya penulisan kode menggunakan TSLint. |
| `yarn sw` | Men-generate dan mengecilkan file service worker PWA ke `build/sw.js`. |
...
1. Wajib atur proxy ke `true`:
   ...
   "host": "https://example.com/"
   ```
2. Dev Server, bagian ini bersifat opsional, boleh dibiarkan untuk edit map, jika tetap dibiarkan jangan lupa jalankan "yarn build-beta".
...
Dan jalankan build standar (bukan beta) "yarn build" atau "yarn build-fast".

Pastikan sudah menjalankan "yarn build-beta" jika mempertahankan server dev atau "yarn build-fast" jika sudah menghapus server dev dari config.json.
...
2. **Jalankan kluster server**:
   Pastikan Anda telah melakukan *build* server (`yarn build-fast`), kemudian jalankan:
   ```bash
   pm2 start ecosystem.config.js
   ```
...
### 11. Perintah Obrolan (In-Game Chat Commands)
...
Secara standar, Docker menggunakan "yarn build-fast". Silakan sesuaikan Dockerfile pada direktori /docker/ menjadi "yarn build-beta" jika Anda ingin menjalankan fitur beta (--beta) dan tools (/tools/).
