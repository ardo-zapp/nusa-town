...
const scripts = [
	['bootstrap', 'app/app.module#AppModule', 'assets', compilerOptions, 'tsconfig-aot.json', 6, false],
	// ['bootstrap-es', 'app/app.module#AppModule', 'assets', compilerOptionsES, 'tsconfig-aot-es.json', 7, false],
	['bootstrap-admin', 'admin/admin.module#AdminAppModule', 'assets-admin', compilerOptions, 'tsconfig-aot.json', 6, true],
	['bootstrap-tools', 'tools/tools.module#ToolsAppModule', 'assets', compilerOptions, 'tsconfig-aot.json', 6, true],
];

class FilterWarningsPlugin {
	apply(compiler) {
		compiler.hooks.afterCompile.tap('FilterWarningsPlugin', compilation => {
			compilation.warnings = compilation.warnings.filter(warning => {
				const msg = typeof warning === 'string' ? warning : (warning.message || '');
				return !msg.includes("is part of the TypeScript compilation but it's unused");
			});
		});
	}
}
